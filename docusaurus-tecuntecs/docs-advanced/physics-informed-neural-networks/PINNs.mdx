...
The intuition is very simple: If the resulting model is too far from zero at the boundaries, that means the model is pretty wrong! (Remember that we defined our boundary conditions to be zero.) A loss function ought not be unique in most caes, but a sum of squares is a good start: it takes into account the magnitud of negative values and encodes information of the boundary conditions very well.
...
## How does TecunTecs Cloud Platform enter the picture?

There are already very good optimizers that can train a neural network of this sort, which is appealing to engineers without training on Data Science. At TecunTecs, we take care of the cloud-based software and scalability.

Go ahead and test different differential equations, boundary equations, and loss functions—then see the results!
